...
import { Pool } from "pg";
import { ensureDbReady, getPglite, loadOrCreateAuthSecret } from "../db";
...
/**
 * Preview secret must outlive module reloads AND process restarts: PGLite users
 * live on disk, so a new random secret on every `npm run dev` would invalidate
 * sessions (and look like the account vanished). Persist next to the DB.
 */
const globalAuthRef = globalThis as typeof globalThis & {
  __grokAuthPreviewSecret__?: string;
};
function previewAuthSecret(): string {
  globalAuthRef.__grokAuthPreviewSecret__ ??= loadOrCreateAuthSecret(() =>
